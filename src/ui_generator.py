...
4. **Componenti UI** — Lista dei componenti visivi con tipo, elementi e interazioni, specificando chiaramente l'MAPPING XSTATE (es. Bottone "Continua" mappato a EVENTO_X)
...
- Il file deve essere pronto per essere usato da un AI UI generator (come v0 o Claude).
"""
...
def discover_screens_llm(machine: dict, context: str, spec: str) -> list:
    ...
    prompt = f"""Sei un Senior Product Manager. Analizza il contesto del progetto e la macchina a stati per determinare quali schermate reali (views) generare.
...
## Istruzioni
Identifica le schermate fisiche del prodotto (es. Login, Dashboard). Per ogni schermata, indica:
1. Nome screen (es. "01_login")
2. Quali stati astratti della macchina sono racchiusi o correlati a questa schermata

Rispondi SOLO con un JSON array valido. Esempio:
[
  {"name": "01_login", "states": ["app_idle", "authenticating", "sessione_scaduta"]},
  {"name": "02_dashboard", "states": ["dashboard_ready", "caricamento_dashboard"]}
]
Non aggiungere markdown come ```json, stampa solo l'array.
"""
    
    system_prompt = "Sei un analista funzionale. Rispondi strettamente con JSON puro."
    ...
    try:
        ...
        print(f"  ⚠️  Errore discover_screens: {e}, uso fallback di default")
        return [
            {"name": "01_login", "states": ["app_idle", "caricamento_auth", "errore_auth", "sessione_scaduta", "access_denied"]},
            {"name": "02_dashboard", "states": ["dashboard_ready", "caricamento_dashboard", "vuoto", "errore_generale", "caricamento"]},
            {"name": "03_catalogo", "states": ["catalogo_ready", "caricamento_catalogo"]},
            {"name": "04_offerte", "states": ["offerte_ready", "caricamento_offerte", "gruppo_in_corso"]},
            {"name": "05_benchmark", "states": ["benchmark_ready", "caricamento_benchmark"]}
        ]
...
def generate_screen_spec_llm(screen_name: str, related_states: list, machine: dict, context: str, spec: str) -> str:
    ...
    prompt = f"""Sei un Senior Product Manager e UI/UX Designer. Analizza questi stati e genera una specifica UI completa per la schermata finale.
...
2. **Dati Necessari** — Mock data realistici.
3. **Componenti UI** — Lista componenti. Mappa in modo RIGIDO ogni bottone/azione all'evento XState corrispondente.
4. **Stati UI** — Come appare la schermata in fase di caricamento, errore o vuoto, basandoti sugli stati correlati forniti.
5. **Note per Generatori AI (v0 / Claude)** — Istruzioni di stile specifiche (Tailwind, Shadcn).

Il file deve essere pronto per il copia-incolla dentro v0.dev.
"""

    system_prompt = "Sei un esperto UI/UX. Crea Blueprint Markdown."
    return call_llm(prompt, system_prompt)
...
def generate_index_llm(states: dict, screens: list, machine: dict) -> str:
    ...
    prompt = f"""Genera un file README.md di indice per questo UI Kit.
Stati Macchina: {states_list}
Schermate Create: {screens_list}

Includi:
1. Titolo e intro
2. Mappatura Schermate
3. Istruzioni su come usare i file Markdown con v0.dev o Claude Artifacts per generare il codice frontend.
"""
    system_prompt = "Sei un technical writer."
    return call_llm(prompt, system_prompt)


def generate_plantuml(machine: dict) -> str:
    """Genera il codice PlantUML flat syntax."""
    uml = "@startuml\n"
    uml += "skinparam state {\n"
...
def main():
    ...
    parser.add_argument("--base-url", default=LLM_BASE_URL, help="Base URL LLM")
    ...
    print(f"🤖 Configurazione: Provider={LLM_PROVIDER} | Model={LLM_MODEL}")
    
    # INIZIALIZZAZIONE ARRAYS CORRETTA
    generated_states = []
    generated_screens = []
    ...
    print(f"\n🎉 Completato! {len(generated_states)} stati e {len(generated_screens)} schermate generate in {args.output_dir}.")
